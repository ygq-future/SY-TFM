...
- 用户可创建、编辑、删除、克隆主机配置
- 每个主机包含：名称、协议类型、地址、端口、用户名、密码、标签、下载路径，以及独立的收藏文件夹列表
...
- 路径栏在 Home 左侧提供当前主机的收藏文件夹下拉入口，点击收藏项直接进入对应远程目录
- 每个主机维护独立收藏列表；列表随主机配置通过 Windows 与 Android 的加密保险库同步
- Windows 右键菜单和 Android 收藏入口均支持将文件夹加入收藏；重复加入同一路径不产生重复项
- 仅纯文件夹选择显示 Add favorite：单选文件夹、多选文件夹可用；文件、`..` 父目录和文件夹/文件混合选择不可用
- Android 工具栏的收藏入口位于 Refresh 前，并提供 Home 按钮
...
- 桌面端使用独立编辑器窗口，支持同时打开多个文件且不阻塞主面板；移动端保留嵌入式编辑器
- 自动检测语言（按文件扩展名）
- 支持亮色/暗色主题
- 自动换行，避免长行水平滚动
- 保存时上传到远程服务器
- 编辑期间检测远程文件版本变化；无本地草稿时自动重新加载，有本地草稿时提示冲突并禁止静默覆盖
- **移动端**：虚拟键盘适配，工具栏可滚动
